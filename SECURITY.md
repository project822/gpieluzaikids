...
| 11 | Auth Middleware & Multi-user | Akses halaman terproteksi; login multi-user (DB) + /api/dev | `proxy.js` (halaman) + `app/admin/(panel)/layout.js` (verifikasi ulang server-side) + `lib/auth.js` + `database/models/User.js` + `app/api/dev/users` |
...
- `frame-src https://www.google.com` mengizinkan embed Google Maps di
  `components/user/LocationSection.jsx`.
...
- **Lapisan kedua (defense-in-depth):** `app/admin/(panel)/layout.js` — layout
  panel admin **memverifikasi ulang token JWT di sisi server** (`cookies()` +
  `verifyToken`) untuk setiap halaman `/admin` sebelum merender konten; sesi
  tidak valid → `redirect('/admin/login')`. Dengan ini, akses langsung
  (forced browsing, mis. mengetik `/admin/events` di browser) tetap tertutup
  meskipun `proxy.js` tidak aktif pada deployment tertentu.
...
- **Project /dev:** `POST|GET /api/dev/users` (file `app/api/dev/users/route.js`)
  ...
  ditolak `401`. **Di produksi, `/api/dev/*` hanya aktif bila diset eksplisit
  `DEV_API_ENABLED=1`** (fail-closed — `lib/devApi.js`).
...
- **Font lokal** — Inter (400–700) + Hanken Grotesk (600–800) via
  `next/font/local` (`fonts/`): tanpa permintaan Google Fonts, semua weight
  memang dipakai. `poweredByHeader: false` di next.config.

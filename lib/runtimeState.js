...
async function getState() {
  // Baca dua konfigurasi MongoDB secara PARALEL — jalur proxy (maintenance &
  // blocklist) membaca state ini pada request pertama per jendela cache, jadi
  // memotong latensi DB menjadi satu perjalanan mempercepat setiap halaman.
  const [dbState, dbVerse] = await Promise.all([
    readRuntimeConfig('maintenance'),
    readRuntimeConfig('verse'),
  ]);

...
export function rateLimitLogin(options = {}) {
  ...
  return function check({ ip, username, deviceId }) {
    const now = Date.now();
    ...
    const touched = [];
    for (const key of keys) {
      const entry = store.get(key) || { count: 0, windowStart: now, blockedUntil: 0, devices: [] };
      ...
      // Rekam perangkat (fingerprint/"MAC") pelaku — supaya bisa diblokir juga
      // di blocklist perangkat dashboard /dev secara real-time.
      if (deviceId) {
        if (!Array.isArray(entry.devices)) entry.devices = [];
        // Batas atas per entri (defensif) — penyerang yang memutar banyak ID
        // perangkat tidak bisa menumbuhkan array tanpa batas.
        if (entry.devices.length < 10 && !entry.devices.includes(deviceId)) {
  ...
  for (const [key, entry] of store) {
    ...
    if (Array.isArray(entry.devices)) {
      for (const d of entry.devices) {
        if (d && !existing.devices.includes(d)) existing.devices.push(d);
      }
    }

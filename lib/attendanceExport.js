...
// ---------- Excel bulanan ----------
export async function buildMonthExcel(sessions, monthKey) {
  const { rows, summary } = buildMonthRows(sessions);
  const title = `Rekap Kehadiran ${formatMonthLabel(monthKey)}`;
...
// ---------- PDF bulanan ----------
export function buildMonthPdf(sessions, monthKey) {
  const { rows, summary } = buildMonthRows(sessions);
  const title = `Rekap Kehadiran ${formatMonthLabel(monthKey)}`;
